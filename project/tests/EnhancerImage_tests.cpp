...
TEST_CASE("Try to save an image in different formats", "[correctness]") {
    ...
    img.saveImage("test_input/test_output/inputfile1_testsave.jpg", EnhancerImage::Filetype::jpg);
    img.saveImage("test_input/test_output/inputfile1_testsave.png", EnhancerImage::Filetype::png);
    img.saveImage("test_input/test_output/inputfile1_testsave.bmp", EnhancerImage::Filetype::bmp);

    stbi_info("test_input/test_output/inputfile1_testsave.jpg", &savedW, &savedH, &savedDim);
    REQUIRE( (savedW == 1562 && savedH == 1200 && savedDim == 3) );

    stbi_info("test_input/test_output/inputfile1_testsave.png", &savedW, &savedH, &savedDim);
    REQUIRE( (savedW == 1562 && savedH == 1200 && savedDim == 3) );

    stbi_info("test_input/test_output/inputfile1_testsave.bmp", &savedW, &savedH, &savedDim);
    REQUIRE( (savedW == 1562 && savedH == 1200 && savedDim == 3) );
...
TEST_CASE("Turn image into grayscale, save & load it back to check if everything is correct", "[correctness]") {
    ...
    img.saveImage("test_input/test_output/inputfile1_grayscale.jpg", EnhancerImage::Filetype::jpg);

    EnhancerImage img2("test_input/test_output/inputfile1_grayscale.jpg");
...
TEST_CASE("Apply adaptive thresholding to images", "[correctness]") {
    ...
    for (const auto & entry : fs::directory_iterator(path)) {
        if (!entry.is_directory() && entry.path().filename().string().find("binarized") == std::string::npos) {
            ...
            std::string output = (entry.path().parent_path() / "test_output" / (entry.path().stem().string()+"_binarized.jpg")).string();
            img.saveImage(output, EnhancerImage::Filetype::jpg);
        }
    }
